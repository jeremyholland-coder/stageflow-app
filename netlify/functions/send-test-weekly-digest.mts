    ...
    if (response.ok) {
      ...
      return {
        ...
        body: JSON.stringify({
          ...
          to: TEST_EMAIL,
          analytics: {
            totalPipeline: analytics.totalPipeline,
            expectedRevenue: analytics.expectedRevenue,
            dealsWon: analytics.dealsWonCount,
            dealsLost: analytics.dealsLostCount,
            newDeals: analytics.dealsAddedCount
          }
        })
      };
    } else {
...
async function calculateWeeklyAnalytics(supabase: any, organizationId: string) {
  ...
  const avgProbability = 0.70;
  const expectedRevenue = Math.round(totalPipeline * avgProbability);

  const dealsThisWeek = deals.filter((d: any) => new Date(d.created) >= oneWeekAgo);
  const wonThisWeek = deals.filter((d: any) => d.status === 'won' && new Date(d.last_activity) >= oneWeekAgo);
  const lostThisWeek = deals.filter((d: any) => d.status === 'lost' && new Date(d.last_activity) >= oneWeekAgo);
  ...
  const monthlyProjection = wonThisWeekValue * 4;
  const quarterlyProjection = wonThisWeekValue * 13;
...
function generateDigestEmail(orgName: string, analytics: any) {
  ...
  const html = `
    ...
    <head>
      ...
      <style>
        ...
        .test-badge { display: inline-block; padding: 8px 16px; background: #EF4444; color: white; border-radius: 6px; font-size: 14px; font-weight: 600; margin-top: 12px; }
    ...
    <body>
      <div class="container">
        <div class="header">
          <img src="https://stageflow.startupstage.com/stageflow-logo.png" alt="StageFlow Logo" width="80" height="80" style="margin-bottom: 16px; display: block; margin-left: auto; margin-right: auto;">
        ...
        <div class="content">
          <div class="section">
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; background: linear-gradient(135deg, #F0FDF4 0%, #DCFCE7 100%); padding: 20px; border-radius: 12px; border: 1px solid #BBF7D0; margin-bottom: 8px;">
              ...
              <div class="trend" style="color: ${getTrendColor(analytics.wonTrend)}; font-size: 24px;">${getTrendArrow(analytics.wonTrend)}</div>
            </div>
            ...
            <div style="background: linear-gradient(135deg, #FEF2F2 0%, #FEE2E2 100%); padding: 16px; border-radius: 12px; border: 1px solid #FECACA;">
              ...
              ${analytics.lostDeals.map((d: any) => `<div style="margin-top: 8px; font-size: 13px; color: #991B1B;"><strong>${d.client}</strong> - ${formatCurrency(d.value)}<br><em style="color: #B91C1C;">Reason: ${d.reason}</em></div>`).join('')}
            </div>
            ` : ''}
          </div>
          ...
          <div class="metric-grid">
            ...
            <div class="metric-box" style="background: linear-gradient(135deg, #F0FDF4 0%, #DCFCE7 100%); border-color: #BBF7D0;">
              ...
              <div class="trend" style="color: ${getTrendColor(analytics.newDealsTrend)};">${getTrendArrow(analytics.newDealsTrend)} vs last week</div>
            </div>
          </div>
          ...
          <div class="alert">
            ...
            ${analytics.stagnantDeals.map((d: any) => `
              <div class="deal-card" style="background: white; margin: 8px 0;">
                ...
                <div style="font-size: 12px; color: #6B7280; margin-top: 4px;">${d.stage} • Stagnant for ${d.days} days</div>
              </div>
            `).join('')}
          </div>
          ` : ''}
        ...
        <div class="footer">
          ...
          <p style="margin-top: 16px; font-size: 12px; color: #9CA3AF;">🧪 This is a test email. Regular digests will be sent every Friday at 7:00 PM PST.</p>
        </div>
      </div>
    </body>
    </html>
  ...
  return {
    subject: `[TEST] Weekly Revenue Pipeline Snapshot - ${orgName}`,
    html
  };
}
