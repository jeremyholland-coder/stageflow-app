...
const STALE_DAYS_THRESHOLD = 14; // Days before a deal is considered stale
 ...
 *
 * Now supports two templates:
 * - Admin/Team Leader: Org-wide rollup with leaderboard
 * - Regular User: Individual pipeline view
 */
const handler: Handler = async (event: HandlerEvent, context: HandlerContext) => {
  ...
  try {
    ...
    for (const org of orgs) {
      ...
      // Get organization members with role info
      ...
      if (membersError || !members) {
        ...
        errors.push(`Failed to fetch members for ${org.name}`);
        continue;
      }
      ...
      for (const member of members as any[]) {
        try {
          ...
          // Determine if user is admin/owner or regular user
          const isAdmin = ['owner', 'admin'].includes(member.role);
          const firstName = member.users?.raw_user_meta_data?.first_name ||
                           member.users?.raw_user_meta_data?.name?.split(' ')[0] ||
                           'there';

          let emailContent;
          if (isAdmin) {
            // Admin/Team Leader gets org-wide view
            const analytics = await calculateAdminAnalytics(supabase, org.id);
            emailContent = generateAdminDigestEmail(firstName, analytics);
            console.log(`[weekly-digest] Generated ADMIN template for ${member.users?.email}`);
          } else {
            // Regular user gets personal pipeline view
            const analytics = await calculateUserAnalytics(supabase, org.id, member.user_id);
            emailContent = generateUserDigestEmail(firstName, analytics);
            console.log(`[weekly-digest] Generated USER template for ${member.users?.email}`);
          }
          ...
          if (response.ok) {
            digestsSent++;
            console.log(`[weekly-digest] ✓ Successfully sent ${isAdmin ? 'ADMIN' : 'USER'} digest to ${member.users.email}`);
          } else {
            const errorData = await response.json();
            console.error(`[weekly-digest] ✗ Failed to send to ${member.users.email}:`, errorData);
            errors.push(`Failed to send to ${member.users.email}: ${JSON.stringify(errorData)}`);
          }
        } catch (memberError: any) {
          console.error(`[weekly-digest] Error processing member ${member.user_id}:`, memberError);
          errors.push(`Error processing member ${member.user_id}: ${memberError.message}`);
        }
      }
    }
...
async function calculateAdminAnalytics(supabase: any, organizationId: string): Promise<AdminAnalytics> {
  ...
  try {
    ...
    if (dealsError) {
      console.error('[weekly-digest] Error fetching deals:', dealsError);
    }
    ...
    // Suggested new leads target (based on conversion rate)
    ...
    console.error('[weekly-digest] Error calculating admin analytics:', error);
    throw error;
  }
}
...
async function calculateUserAnalytics(supabase: any, organizationId: string, userId: string): Promise<DigestAnalytics> {
  ...
  try {
    ...
    if (dealsError) {
      console.error('[weekly-digest] Error fetching user deals:', dealsError);
    }
    ...
    console.error('[weekly-digest] Error calculating user analytics:', error);
    throw error;
  }
}
