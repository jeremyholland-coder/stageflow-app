...
export const Settings = () => {
  ...
  const { user, organization, userRole, setActiveView, logout, addNotification, setAvatarUrl, setProfileData } = context || {};
  ...
  // Save profile handler (first_name, last_name)
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-sm text-gray-300">Loading settings...</span>
        </div>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <button
            ...
            className="p-2 border border-gray-700 text-gray-300 hover:text-white hover:bg-gray-800/50 rounded-lg transition"
          >
            <ArrowLeft className="w-5 h-5" />
          </button>
          <div>
            <h1 className="text-large-title text-white">Settings</h1>
            <p className="text-subheadline text-gray-300">{organization?.name || 'Your Organization'}</p>
          </div>
        </div>
          ...
          <SettingCard data-section="pipeline-health">
            ...
            {loadingHealth ? (
              <div className="flex items-center justify-center py-8">
                ...
                <span className="ml-2 text-gray-300">Checking pipeline health...</span>
              </div>
            ) : pipelineHealth ? (
              <div className="space-y-4">
                ...
                <div className="p-6 bg-gradient-to-br from-teal-500/10 to-teal-600/10 rounded-xl border border-teal-500/20">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-4 border-t border-teal-500/20">
                    ...
                    <div>
                      <p className={`text-2xl font-bold ${pipelineHealth.orphanedDeals > 0 ? 'text-red-400' : 'text-gray-300'}`}>
                        {pipelineHealth.orphanedDeals}
                      </p>
                      <p className="text-xs text-gray-300">Orphaned Deals</p>
                    </div>
                    <div>
                      <p className="text-2xl font-bold text-gray-300">{pipelineHealth.totalDeals}</p>
                      <p className="text-xs text-gray-300">Total Deals</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex justify-end">
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 border border-gray-700 text-gray-300 hover:text-white hover:bg-gray-800/50 rounded-lg transition"
                  >
                    <RefreshCw className="w-4 h-4" />
              ...
              <div className="text-center py-8">
                <p className="text-gray-300">Unable to load pipeline health</p>
              </div>
            )}
          </SettingCard>
