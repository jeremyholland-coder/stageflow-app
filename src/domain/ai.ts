 ...
 * AI DOMAIN SPINE
 *
 * Single source of truth for AI provider status, error classification,
 * and response normalization. All AI-related code must go through this module.
 *
 * @module domain/ai
 ...
 * - DEGRADED: Provider is slow or intermittent
 * - DOWN: Provider is completely unavailable
 * - MISCONFIGURED: API key is invalid or expired
...
/**
 * Normalized error info for AI operations.
 * This is what components should consume - not raw provider errors.
 */
export interface AIErrorInfo {
  ...
  retryAfterSeconds?: number; // Suggested wait time
  dashboardUrl?: string;     // Link to provider dashboard
}

/**
 * Normalized AI response.
 * This is what components should render - not raw provider responses.
 */
...
/**
 * Provider dashboard URLs for troubleshooting.
 */
 ...
 * - Strips provider-specific metadata
...
/**
 * Determine provider status from error info.
 */
export function getProviderStatusFromError(error: AIErrorInfo): AIProviderStatus {
  switch (error.code) {
    case 'PROVIDER_DOWN':
      return 'DOWN';
